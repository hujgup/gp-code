...
string StringUtils::substring_replace(const string& str,string from,string to) {
	// Amalgamation of several answers from http://stackoverflow.com/questions/2896600/how-to-replace-all-occurrences-of-a-character-in-string + original code
	stringstream stream;
	size_t fromLength = from.length();
	size_t delta;
	size_t oldIndex = 0;
	size_t index = str.find(from,0);
	while (index != string::npos) {
		delta = index - oldIndex;
		if (delta > 0) {
			stream << str.substr(index,delta);
		}
		stream << to;
		index += fromLength;
		oldIndex = index;
	...
	if (oldIndex < str.length() - 1) {
		delta = str.length() - oldIndex;
		stream << str.substr(oldIndex,delta);
	} else if (oldIndex == 0 && str.length() == 1) {
		stream << str;
	}
	return string(istreambuf_iterator<char>(stream),{});
}
bool StringUtils::starts_with(string str,char c) {
